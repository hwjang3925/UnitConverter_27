...
→ 11 passed in 0.94s (7 acceptance + 4 unit)
```

**HTML 요약:** [`Report/green-test-results.html`](green-test-results.html) — 브라우저에서 한눈에 확인  
**터미널 가이드:** [`doc/terminal-self-test.md`](../doc/terminal-self-test.md) — 명령·검증 로그

| Suite | Count | Status | Verified duration |
|-------|-------|--------|-------------------|
| acceptance (`tests/acceptance/`) | 7 | Green | 0.92s |
...
| Speaker | Content |
|---------|---------|
| AI | 가이드 제공 — pytest 전체/분리, CLI pipe smoke test |
| AI | Agent 실행 — **11/11 passed**, CLI 3케이스 출력 확인 |
| AI | `doc/terminal-self-test.md`, `green-test-results.html` § Terminal verification 반영 |

**CLI smoke (PowerShell):**

| Input | Output (요약) | OK |
|-------|---------------|-----|
| `meter:2.5` | 3줄 변환 (8.2 feet, 2.7 yard) | ✅ |
| `meter:-1` | `Invalid number: -1` | ✅ |
| `meter2.5` | format error message | ✅ |
